...
int Load_Param_Ai_OBJECT_TRACK(const char * file)
{
    ...
    if (ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG, "[%s][model_id_sot] Fail to convert string name [%s] to enum number!\n", tmp_section, str_name);
    } else {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG, "[%s][model_id_sot] Convert string name [%s] to enum number [%d].\n", tmp_section, str_name, enum_num);
