...
int Load_Param_Ai_Img_Txt_Clip(const char * file)
{
    ...
    if (ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG, "[%s][model_id_img] Fail to convert string name [%s] to enum number!\n", tmp_section, str_name);
    } else {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG, "[%s][model_id_img] Convert string name [%s] to enum number [%d].\n", tmp_section, str_name, enum_num);
    ...
    ret = app_ipcam_Param_Convert_StrName_to_EnumNum(str_name, ai_supported_model, TDL_MODEL_MAX, &enum_num);
    if (ret != CVI_SUCCESS) {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG, "[%s][model_id_txt] Fail to convert string name [%s] to enum number!\n", tmp_section, str_name);
    } else {
        APP_PROF_LOG_PRINT(LEVEL_DEBUG, "[%s][model_id_txt] Convert string name [%s] to enum number [%d].\n", tmp_section, str_name, enum_num);
        Ai->model_id_txt = enum_num;
    }

    APP_PROF_LOG_PRINT(LEVEL_DEBUG, "model_id_img=%d model_path_img=%s\n",
        Ai->model_id_img, Ai->model_path_img);

    APP_PROF_LOG_PRINT(LEVEL_DEBUG, "model_id_txt=%d model_path_txt=%s\n",
        Ai->model_id_txt, Ai->model_path_txt);

    APP_PROF_LOG_PRINT(LEVEL_DEBUG, "model_path_txt=%s\n", Ai->model_path_cfg);

    APP_PROF_LOG_PRINT(LEVEL_DEBUG, "bEnable=%d Grp=%d Chn=%d GrpW=%d GrpH=%d txt_dir=%s\n",
